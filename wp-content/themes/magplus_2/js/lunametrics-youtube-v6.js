...
function trackYouTube()
{
	...
	jQuery('iframe').each(function() {
		console.log("# of iframes= "+$(this).attr('src'));
		//but what is this?
		//an iframe! Avast!
		if($(this).attr('src')){
			...
			if(reloadFrames){
				...
				if(SourceCheckA[2]=="?"){
					...
					if(SourceCheckB){
						console.log("has he gift");
						//it has the gift
						//accept it and move on
					}else{
						console.log("adding enableJSapi");
...
$('.what-is-vid.colorbox-iframe').colorbox({
  onComplete:function() {
  	delayTrack=setTimeout(function(){
			    console.log('ifrem= '+$('.cboxIframe').length);
				  	$('iframe.cboxIframe').load(function() {
				        trackYouTube();
				    });
				clearTimeout(delayTrack);
			},500);
  }
});
	...
	for (j=0; j<videoArray.length; j++) {
	    ...
	    if (videoArray[j]==videoID) {
            ...
            if (event.data == YT.PlayerState.PLAYING) {
				...
				console.log('playing youtube vid '+thisVideoTitle);
